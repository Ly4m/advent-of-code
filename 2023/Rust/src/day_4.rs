...
fn parse_line_to_game(line: &str) -> Game {
    ...
    let id = game_parts[0].replace("Card", "").trim().parse::<i32>().unwrap();
...
pub fn solve_part_2(test_mode: bool) -> usize {
    ...
    games
        ...
        .for_each(|g| {
            ...
            let end_range = if (g.id + count) > (games.len() + 1) as i32 { (games.len() + 1) as i32 } else { g.id + count };
